...
\newcommand{\ipfourhdrnew}[2]{\cbgFour IPv4 & %
    \begin{tikzpicture}%
        \node[align=left, inner sep=0] (xx) {\\[2pt]SA:~\texttt{#1}\\DA:~\texttt{#2}};%
    \end{tikzpicture}\\[3pt]\hline}

%\newcommand{\ipsixhdr}[2]{\cbgSix IPv6 & (\texttt{#1}, \texttt{#2})\\[3pt]\hline}
\newcommand{\ipsixhdrnew}[2]{\cbgSix IPv6 & %
    \begin{tikzpicture}%
        \node[align=left, inner sep=0, yshift=1.5pt] (xx) {\\[2pt]SA:~\texttt{#1}\\DA:~\texttt{#2}};%
    \end{tikzpicture}\\[3pt]\hline}
...
\newcommand{\ethhdrnewer}[2]{\cbgSRH Eth & %
    \begin{tikzpicture}%
        \node[align=left, inner sep=0, yshift=1.5pt] (xx) {\\[2pt]#1};%
    \end{tikzpicture}\\[3pt]\hline}

\newcommand{\mplshdrnewer}[1]{\cbgSRH MPLS & %
    \begin{tikzpicture}%
        \node[align=left, inner sep=0, yshift=1.5pt] (xx) {\\[2pt]#1};%
    \end{tikzpicture}\\[3pt]\hline}
...
\newcommand{\ipfourmplspktnew}[4][\stdpktwidth]{%
    \begin{srv6packetnew}[#1]%
    \mplshdrnewer{#2}
        \ipfourhdrnew{#3}{#4}%
    \end{srv6packetnew}
