...
\begin{scope}[shift={(#1)}]
    % make fake node to measure the width of the header text
    \node[align=left, text opacity=0, inner sep=0, anchor=south west] (sample) {#5\\#7};
    % measure it
    \tikzmath{
        coordinate \wNode;
    ...
    % https://tex.stackexchange.com/questions/655950/how-to-store-node-width-in-a-variable
\def\minwidthleft{\n1}
...
\begin{scope}[shift={(#1)}]
    % make fake node to measure the width of the header text
    \node[align=left, text opacity=0, inner sep=0, inner sep=0, anchor=south west] (sample) {#5\\#7};
    % measure it
    \tikzmath{
        coordinate \wNode;
        \wNode1=(sample.east) - (sample.west);
